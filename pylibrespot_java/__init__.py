...
class LibrespotJavaAPI:
    ...
    async def start_websocket_handler(
        ...
        try:
            ...
            await self.start_websocket_handler(
                ws_port, update_callback, websocket_reconnect_time
            )

    def _debug_string(self,string_base,status) -> str:
    ...
    async def player_load(self, uri, play) -> int:
        """Load track from URI."""
        resp = await self.post_request(endpoint="player/load",data={'uri':uri,'play':play})
        if resp.status in [204,500,503]:
            _LOGGER.debug(self._debug_string("Unable to load track.",resp.status))
        return resp.status
    ...
    async def player_pause(self) -> int:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to pause player.", resp.status))
        return resp.status
    ...
    async def player_resume(self) -> int:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to resume player.", resp.status))
        return resp.status
    ...
    async def player_next(self) -> int:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to skip to the next track.", resp.status))
        return resp.status
    ...
    async def player_prev(self) -> int:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to skip to the previous track.", resp.status))
        return resp.status

    async def player_set_volume(self,volume: int) -> int:
        """Set volume to a given volume between 0 and 65536."""
        resp = await self.post_request(endpoint="player/set-volume",data={'volume':volume})
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to set the volume.", resp.status))
        return resp.status
    ...
    async def player_volume_up(self) -> int:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to turn the volume up.", resp.status))
        return resp.status
    ...
    async def player_volume_down(self) -> int:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to turn the volume down.", resp.status))
        return resp.status
    ...
    async def player_current(self) -> dict:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string("Unable to retrieve information about the current track.", resp.status))
    ...
    async def metadata(self,uri: str) -> dict:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string(f"Unable to get metadata for {uri}.", resp.status))
        json = await resp.json(content_type=None)
        return json
    ...
    async def search(self,query: str) -> dict:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string(f"Unable to search for {query}.", resp.status))
        json = await resp.json(content_type=None)
        return json
    ...
    async def token(self,scope: str) -> dict:
        ...
        if resp.status in [204, 500, 503]:
            _LOGGER.debug(self._debug_string(f"Unable to get token for {scope}.", resp.status))
        json = await resp.json(content_type=None)
        return json
...
async def main():
    with aiohttp.ClientSession() as session:
        api=LibrespotJavaAPI(session,'10.0.0.134',24879)
        await api.player_resume()